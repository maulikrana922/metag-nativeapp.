...
import { Modal,View, Text, Alert,Button,TextInput,StyleSheet,TouchableWithoutFeedback,Keyboard, TouchableOpacity } from 'react-native';
...
//   }, []);
  ...
  const fn = async () => {
    const obj = await JSON.stringify({name:text,address:text2,number:text3})
    // writeNdef()
  ...
  return (
    <SafeAreaView style={{marginTop:'auto',marginBottom:'auto'}}>
      {visible===true && <Modal transparent={true} visible={visible}>
        {console.log("vvv",visible)}
        {console.log("in modal")}
        <View style={{flex:1,justifyContent:"center"}}>
          {console.log("asdsdsd",obj2,typeof obj2)}
          <View style={{margin:20,backgroundColor:'#fff',height:'50%',display:'flex',justifyContent:'center'}}>
         <Text style={{fontSize:30,textAlign:'center'}}>name: {obj2.name}</Text>
         <Text style={{fontSize:30,textAlign:'center'}}>your address: {obj2.address}</Text>
         <Text style={{fontSize:30,textAlign:'center'}}>number: {obj2.number}</Text>
         <TouchableOpacity onPress={() => setVisible(false)} style={{marginTop:"10%",backgroundColor:"pink",width:'30%',height:'10%',marginLeft:'auto',marginRight:'auto'}}>
  ...
  <TextInput onChangeText={setText2} value={text2} style={{
    height: 40,
    margin: 12,
    borderWidth: 1,
    color:'black'
...
<TextInput onChangeText={setText3} value={text3} style={{
    height: 40,
    margin: 12,
    borderWidth: 1,
    color:'black'
        ...
        <Button onPress={()=> fn()} title="write"  color="#841584"  accessibilityLabel="Learn more about this purple button"/>
      </View>
